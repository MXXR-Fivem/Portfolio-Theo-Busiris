...
export default function IconTextButton ({data, onClick}) {
    if (!data) return null;
    return (
        <a style={{"width": "95%", "display": "flex", "justifyContent": "center"}} onClick = {onClick && onClick} href={data.href} target="_blank">
            <button style={{"backgroundColor": data.backgroundColor, "border": data.border}} className={styles.iconTextButton}>
                {data.icon == 1 ? (
                    ...
                    <PiReadCvLogo style={{"color": data.iconColor, "transform": "scale(1.5)"}} className={styles.iconButton}/>
                ) : (
